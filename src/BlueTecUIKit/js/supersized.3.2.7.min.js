  ...
  e.supersized = function (t) {
    ...
    r._build = function () {
      ...
      while (t <= r.options.slides.length - 1) {
        ...
        if (t == r.options.start_slide - 1) {
          r.options.slide_links &&
            (i = `${i}<li class="slide-link-${t} current-slide"><a>${s}</a></li>`);
      ...
      r.options.thumb_links &&
        ...
        e(vars.thumb_tray).append(`<ul id="${vars.thumb_list.replace('#', '')}">${o}</ul>`);
      e(r.el).append(n);
    ...
    r._start = function () {
      r.options.start_slide
        ...
        : (vars.current_slide = Math.floor(Math.random() * r.options.slides.length));
      const t = r.options.new_window ? ' target="_blank"' : '';
      ...
      if (r.options.slides.length > 1) {
        if (r.options.slides.length > 2) {
          ...
          const u = e(`<img src="${r.options.slides[loadPrev].image}"/>`);
          const a = `${r.el} li:eq(${loadPrev})`;
          u.appendTo(a)
            .wrap(`<a ${o}${t}></a>`)
            .parent()
            .parent()
      ...
      const f = e(`<img src="${api.getField('image')}"/>`);
      const l = `${r.el} li:eq(${vars.current_slide})`;
      f.appendTo(l)
        .wrap(`<a ${o}${t}></a>`)
        .parent()
        .parent()
      ...
      if (r.options.slides.length > 1) {
        ...
        const c = e(`<img src="${r.options.slides[loadNext].image}"/>`);
        const h = `${r.el} li:eq(${loadNext})`;
    ...
    r.launch = function () {
      ...
      r.options.keyboard_nav &&
        e(document.documentElement).keyup((e) => {
      ...
      if (r.options.slideshow && r.options.slides.length > 1) {
        r.options.autoplay && r.options.slides.length > 1
          ...
          : (vars.is_paused = !0);
        e('.load-item img').bind('contextmenu mousedown', () => !1);
      }
      e(window).resize(() => {
        r.resizeNow();
      });
    ...
    r.resizeNow = function () {
      return r.$el.each(() => {
        ...
        r.options.image_protect && e('img', r.el).bind('contextmenu mousedown', () => !1);
        return !1;
      });
    };
    r.nextSlide = function () {
      ...
      vars.in_animation = !0;
      clearInterval(vars.slideshow_interval);
      const t = r.options.slides;
      const n = r.$el.find('.activeslide');
      ...
      vars.current_slide + 1 == r.options.slides.length
        ...
        : vars.current_slide++;
      const i = e(`${r.el} li:eq(${vars.current_slide})`);
      const s = r.$el.find('.prevslide');
      r.options.performance == 1 && r.$el.removeClass('quality').addClass('speed');
      loadSlide = !1;
      vars.current_slide == r.options.slides.length - 1
        ? (loadSlide = 0)
        : (loadSlide = vars.current_slide + 1);
      const o = `${r.el} li:eq(${loadSlide})`;
      if (!e(o).html()) {
        const u = r.options.new_window ? ' target="_blank"' : '';
        imageLink = r.options.slides[loadSlide].url
          ? `href='${r.options.slides[loadSlide].url}'`
          : '';
        const a = e(`<img src="${r.options.slides[loadSlide].image}"/>`);
        a.appendTo(o)
    ...
    r.goTo = function (t) {
      if (vars.in_animation || !api.options.slideshow) return !1;
      const n = r.options.slides.length;
      t < 0 ? (t = n) : t > n && (t = 1);
      t = n - t + 1;
      clearInterval(vars.slideshow_interval);
